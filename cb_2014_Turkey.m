function [median, sigma, period1] = cb_2014_Turkey(T,rup,sitevar)
...
% if Zhyp is unknown...
if isempty(rup.Zhyp) && ~isempty(rup.W)
    if rup.M < 6.75
...
if length (T) == 1 && T == 1000
    ...
    for ipT=1:length(period1)
        [median(ipT),sigma(ipT)]=CB_2014_nga_sub(rup,sitevar,ipT,Frv,Fnm,A1100,Ztor,Zhyp);
        [PGA,~]=CB_2014_nga_sub(rup,sitevar,22,Frv,Fnm,A1100,Ztor,Zhyp);
        if median(ipT)<PGA && period1(ipT)<0.25
            median(ipT)= PGA;
        end
    end
% Compute Sa and sigma with user-defined period    
    ...
    for i=1:length(T)
        ...
        if ( isempty(find(abs(period-Ti) < 0.0001, 1))) % The user defined period requires interpolation
            ...
            [Sa_low, sigma_low] = CB_2014_nga_sub (rup,sitevar,ip_low,Frv,Fnm,A1100,Ztor,Zhyp);
            [Sa_high, sigma_high] = CB_2014_nga_sub (rup,sitevar,ip_high,Frv,Fnm,A1100,Ztor,Zhyp);
            [PGA,~]=CB_2014_nga_sub (rup,sitevar,22,Frv,Fnm,A1100,Ztor,Zhyp);
            ...
            if median(i)<PGA && period1(i)<0.25
                median(i)= PGA;
            end
        else
            ip_T = find(abs((period- Ti)) < 0.0001);
            [median(i), sigma(i)] = CB_2014_nga_sub(rup,sitevar,ip_T,Frv,Fnm,A1100,Ztor,Zhyp);
            [PGA,~]=CB_2014_nga_sub(rup,sitevar,22,Frv,Fnm,A1100,Ztor,Zhyp);
...
function [Sa, sigma] = CB_2014_nga_sub (rup,sitevar,ip,Frv,Fnm,A1100,Ztor,Zhyp)
M = rup.M;
Rrup = rup.Rrup;
Rjb = rup.Rjb;
Rx = rup.Rx;
...
if sitevar.region == 5
    region = 3;
elseif sitevar.region == 6
    region = 0;
else
    region = sitevar.region;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%  modified for Turkey  %%%%%%%%%%%%%%%%%%%%%%%%%% 
region = 3;  % representing Turkey
c0 = [-2.2125  -2.0959  -1.6408  -0.8309  -0.0987  -0.4371  -1.779  -2.6662  -3.5286  -4.3205  -5.9444  -6.9932  -8.4486  -10.1845  -11.7739  -12.4331  -12.8128  -13.2324  -13.4838  -15.9345  -17.0729  -2.2746  -2.0735];
c1 = [0.5026  0.4830  0.4104  0.3138  0.2192  0.3137  0.6154  0.7825  0.9156  1.0296  1.3322  1.5016  1.6676  1.8979  2.0595  2.1028  1.9952  1.8824  1.7854  2.2288  2.1825  0.5086  1.2489];
c3 = [-0.8081  -0.7986  -0.7993  -0.6908  -0.6031  -0.6563  -0.7088  -0.8193  -0.8043  -0.8910  -0.8600  -0.8823  -1.0441  -1.0253  -1.0796  -1.0867  -0.9128  -0.9599  -1.0930  -0.5208  -0.5705  -0.8185  -0.6311];
c5 = [-2.8934  -2.8916  -2.9141  -2.9445  -2.9043  -2.7820  -2.6003  -2.5500  -2.5081  -2.4626  -2.3752  -2.3422  -2.2900  -2.1547  -2.0837  -2.1109  -2.0454  -1.9353  -1.9399  -2.1417  -2.0739  -2.8892  -2.4717];
c11 = [0.8813  0.9269  1.0151  1.1396  1.3120  1.5024  1.8325  2.0690  2.2050  2.3060  2.3980  2.3550  1.9950  1.4470  0.3927  -0.4251  -0.6484  -0.5308  -0.4442  -0.3015  -0.2530  0.8776  1.6551];
c17 = [0.0327  0.0331  0.0352  0.0382  0.0401  0.0382  0.0351  0.0307  0.0261  0.0222  0.0199  0.0151  0.0092  0.0130  0.0070  -0.0029  -0.0113  -0.0135  -0.0170  -0.0217  -0.0049  0.0325  0.0201];
c18 = [0.0327  0.0331  0.0352  0.0382  0.0401  0.0382  0.0351  0.0307  0.0261  0.0222  0.0199  0.0151  0.0092  0.0130  0.0070  -0.0029  -0.0113  -0.0135  -0.0170  -0.0217  -0.0049  0.0325  0.0201];
c20 = [-0.00387  -0.00387  -0.00379  -0.00388  -0.00419  -0.00507  -0.00518  -0.00523  -0.00531  -0.00528  -0.00437  -0.00387  -0.00268  -0.00202  -0.00082  0.00000  0.00000  0.00000  0.00000  0.00000  0.00000  -0.00387  -0.00083];

Phi_s2s = [0.407 	0.408 	0.410 	0.440 	0.479 	0.486 	0.479 	0.465 	0.447 	0.434 	0.425 	0.427 	0.443 	0.476 	0.500 	0.519 	0.476 	0.452 	0.420 	0.352 	0.313 	0.406 	0.324];
Phi_ss = [0.493 	0.493 	0.499 	0.509 	0.517 	0.510 	0.513 	0.513 	0.519 	0.524 	0.521 	0.512 	0.494 	0.476 	0.450 	0.437 	0.413 	0.387 	0.374 	0.398 	0.401 	0.493 	0.465];
tau1 = [0.437 	0.438 	0.445 	0.456 	0.466 	0.475 	0.468 	0.463 	0.441 	0.432 	0.417 	0.410 	0.399 	0.391 	0.398 	0.388 	0.394 	0.412 	0.413 	0.392 	0.387 	0.436 	0.378];
tau2 = [0.283 	0.285 	0.291 	0.311 	0.333 	0.353 	0.367 	0.362 	0.357 	0.348 	0.324 	0.299 	0.246 	0.197 	0.189 	0.181 	0.233 	0.292 	0.393 	0.503 	0.573 	0.282 	0.230];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
%        site1100 = site(sitevar.is_soil,1100,0,Z25A,sitevar.Z10,sitevar.Zbot,sitevar.region);
        site1100.Vs30 = 1100;
        site1100.Z25 = Z25A;
        site1100.region = region;
       A1100 = cb_2014_Turkey(0,rup,site1100);
    end
