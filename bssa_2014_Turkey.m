function [median, sigma, period1] = bssa_2014_Turkey(T, rup, site)
...
% Shallow Crustal Earthquakes.¡± Earthquake Spectra, 30(3), 1057¨C1085.
...
if site.region == 5  % Mapping generic region inputs to model specific values
    region = 3;
elseif site.region == 6
    region = 0;
else
    region = site.region;
end
...
if rup.lambda <= 30 && rup.lambda >= -30 || rup.lambda <= 180 && rup.lambda >= 150 || rup.lambda <= -150 && rup.lambda >= -180  
    FaultType = 1; %Strike-slip
elseif rup.lambda <= -30
    FaultType = 2; %Normal
elseif rup.lambda >= 30
    FaultType = 3; %Reverse
else
    FaultType = 0; %Other/unspecified
end   
U = (FaultType == 0);
SS = (FaultType == 1);
NS = (FaultType == 2);
...
if length (T) == 1 && T == 1000 % Compute median and sigma with pre-defined period
    ...
    for ip=3:length(period)
        [median(ip-2),sigma(ip-2)]=BSSA_2014_sub(rup.M, ip, rup.Rjb, U, SS, NS, RS, region, site.Z10, site.Vs30);
    end
    ...
    for i=1:length(T)
        ...
        if ( isempty(find(abs(period-Ti) < 0.0001, 1))) % The user defined period requires interpolation
            ...
            [sa_low, sigma_low] = BSSA_2014_sub(rup.M, ip_low, rup.Rjb, U, SS, NS, RS, region, site.Z10, site.Vs30);
            [sa_high, sigma_high] = BSSA_2014_sub(rup.M, ip_high, rup.Rjb, U, SS, NS, RS, region, site.Z10, site.Vs30);
            ...
            [median(i), sigma(i)] = BSSA_2014_sub(rup.M,ip_T,rup.Rjb,U,SS,NS,RS,region,site.Z10,site.Vs30);
        end
    end
end
...
c0 = [0.8376  0.5727  0.5906  0.6365  0.7418  1.0459  1.2331  1.1643  0.8234  0.6189  0.4405  0.2878  0.0536  -0.0718  -0.2639  -0.3622  -0.2563  -0.322  -0.234  -0.5045  -0.5999  -0.7273  -0.9382];
e4 = [0.907  0.9238  0.9145  0.9248  0.921  0.867  0.8499  0.8507  0.7138  0.592  0.4931  0.4774  0.5088  0.5723  0.8937  1.1798  1.5501  1.7767  2.0399  2.1781  2.245  2.1443  1.9891];
c = [-0.8548  -0.6  -0.6037  -0.5739  -0.5341  -0.458  -0.4441  -0.4881  -0.5239  -0.5815  -0.6258  -0.6753  -0.7531  -0.8175  -0.87  -0.8936  -0.8519  -0.8166  -0.6985  -0.6081  -0.5078  -0.3258  -0.2618];
c1 = [-1.4554  -1.5275  -1.5333  -1.5428  -1.5782  -1.665  -1.7036  -1.667  -1.5326  -1.4466  -1.3834  -1.3342  -1.2685  -1.2299  -1.156  -1.1165  -1.1101  -1.1129  -1.1235  -1.0824  -1.0408  -1.0364  -1.0573];
c3 = [-0.00079  -0.00289  -0.00288  -0.00284  -0.00274  -0.00275  -0.00318  -0.00356  -0.00409  -0.00412  -0.00398  -0.00372  -0.00321  -0.00281  -0.00203  -0.00151  -0.00067  -0.00013  0.00004  -0.00045  -0.0007  -0.0003  -0.00004];
mh = [6.7  6.2  6.2  6.2  6.2  6.2  6.2  6.2  6.4  6.6  6.7  6.7  6.7  6.7  6.6  6.5  6.4  6.3  6.3  6.2  6.2  6.2  6.2];

Phi_s2s = [0.332  0.406  0.407  0.407  0.409  0.44  0.484  0.488  0.482  0.468  0.452  0.438  0.432  0.432  0.449  0.485  0.506  0.523  0.479  0.453  0.418  0.355  0.319];
Phi_ss = [0.466  0.494  0.494  0.494  0.5  0.511  0.513  0.513  0.518  0.52  0.528  0.533  0.525  0.514  0.489  0.471  0.441  0.423  0.401  0.378  0.37  0.393  0.395];
tau1 = [0.383  0.442  0.443  0.442  0.444  0.452  0.468  0.485  0.48  0.468  0.45  0.443  0.431  0.422  0.403  0.389  0.394  0.377  0.389  0.409  0.405  0.411  0.399];
tau2 = [0.312  0.319  0.319  0.323  0.328  0.342  0.367  0.388  0.417  0.425  0.425  0.426  0.392  0.357  0.321  0.276  0.228  0.216  0.281  0.325  0.419  0.497  0.569];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 


%% The source(event function):
% include c0 in the original F_E term
if M <= mh (ip)
    F_E = c0(ip) + e0(ip) * U + e1(ip) * SS + e2(ip) * NS + e3(ip) * RS + e4(ip) * (M - mh(ip)) + e5(ip) * (M - mh(ip))^2;
else
    F_E = c0(ip) + e0(ip) * U + e1(ip) * SS + e2(ip) * NS + e3(ip) * RS + e6(ip) * (M - mh(ip));
end
